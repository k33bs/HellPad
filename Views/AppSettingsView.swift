...
struct ControlsTabView: View {
    ...
    var body: some View {
        ScrollView {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Toggle(isOn: Binding(
                    get: { stratagemManager.stratagemCompanionEnabled },
                    set: {
                        stratagemManager.stratagemCompanionEnabled = $0
                        if $0 {
                            stratagemManager.requestStratagemCompanionScreenCapturePermissionIfNeeded()
                        }
                        stratagemManager.saveAllSettings()
                    }
                )) {
                    Text("Stratagem Companion")
                        .font(.subheadline)
                        .fontWeight(.medium)
                }
                .toggleStyle(.switch)

                Text("Read stratagem availability from the game UI")
                    .font(.caption)
                    .foregroundColor(.secondary)

                if stratagemManager.stratagemCompanionEnabled && !stratagemManager.stratagemCompanionHasScreenCapturePermission {
                    HStack(spacing: 8) {
                        Text("Screen Recording permission is required. If you clicked ‘Don’t Allow’, enable it in System Settings.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                        Spacer()
                        Button("Open System Settings") {
                            stratagemManager.openStratagemCompanionScreenCaptureSystemSettings()
                        }
                        .controlSize(.small)
                    }
                }
                ...
                Toggle(isOn: Binding(
                    get: { stratagemManager.stratagemCompanionDebugWindowEnabled },
                    set: {
                        stratagemManager.stratagemCompanionDebugWindowEnabled = $0
                        stratagemManager.saveAllSettings()
                    }
                )) {
                    Text("Show Companion Debug Window")
                        .font(.subheadline)
                        .fontWeight(.medium)
                }
                .toggleStyle(.switch)

                Text("Show detected stratagems and cooldowns in a small window")
                    .font(.caption)
                    .foregroundColor(.secondary)

#if DEBUG
                Toggle(isOn: Binding(
                    get: { stratagemManager.stratagemCompanionLiveOcrWindowEnabled },
                    set: {
                        stratagemManager.stratagemCompanionLiveOcrWindowEnabled = $0
                    }
                )) {
                    Text("Show Live OCR Window")
                        .font(.subheadline)
                        .fontWeight(.medium)
                }
                .toggleStyle(.switch)

                Text("Debug-only: Show raw OCR output for troubleshooting")
                    .font(.caption)
                    .foregroundColor(.secondary)
#endif
            }
            .onAppear {
                stratagemManager.refreshStratagemCompanionScreenCapturePermissionState()
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                Toggle(isOn: Binding(
                    get: { stratagemManager.reloadMonitorEnabled },
                    set: {
                        stratagemManager.reloadMonitorEnabled = $0
                        if $0 {
                            stratagemManager.requestReloadMonitorScreenCapturePermissionIfNeeded()
                        }
                        stratagemManager.saveAllSettings()
                    }
                )) {
                ...
                Text("Speak a warning when the HUD shows your weapon needs reloading")
                    .font(.caption)
                    .foregroundColor(.secondary)

                if stratagemManager.reloadMonitorEnabled && !stratagemManager.reloadMonitorHasScreenCapturePermission {
                    HStack(spacing: 8) {
                        Text("Screen Recording permission is required. If you clicked ‘Don’t Allow’, enable it in System Settings.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                        Spacer()
                        Button("Open System Settings") {
                            stratagemManager.openReloadMonitorScreenCaptureSystemSettings()
                        }
                        .controlSize(.small)
                    }
                }
            ...
            .onAppear {
                stratagemManager.refreshReloadMonitorScreenCapturePermissionState()
            }
